...
This model is a linear model because the relationship between the response variable, height, and the explanatory variable, time, is linear (See @fig-model-1). In a linear model, the gradient of the line is the same no matter what the value of $t$. In this case, it is fixed at 2 cm per day. 
...
This model is not a straight line (See @fig-model-2). The gradient of the line increase as time goes on.
