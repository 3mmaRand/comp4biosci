...
7.  We use the `lm()` function to fit a linear model in R. The
    `summary()` function gives us the *p*-values and *R*-squared value
    and the `plot()` function gives us diagnostic plots to check the
    assumptions.

8.  When reporting the results of statistical tests give the
