...
There are some rank tests (e.g., Friedman, Scheirer Ray Hare) for
specific situations but there are no non-parametric alternatives as
directly equivalantas the Kruskal-Wallis is for the one-way ANOVA.

In these cases you have three other good options.

1.  **Using non-parametric one-way tests**. Divide the dataset according
...
3.  **Use two-way ANOVA carefully**. Using the two-way ANOVA test 
