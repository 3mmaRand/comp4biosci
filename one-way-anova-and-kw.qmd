...
-   Use common sense - the response should be continuous (or nearly
    continuous, see Ideas about data: Theory and practice (see
    @sec-ideas-about-data-theory-practice). Consider whether you would
    expect the response to be continuous
...
(see [@sec-workflow-rstudio-creating-rsp]), add a `data-raw` folder and
open a new script. You will also need to load the **`tidyverse`**
package [@tidyverse].
...
Both versions of the null hypothesis describe the same idea: that here
is **no real difference** between the groups.

-   **Using** $\beta$ coefficients: This version focuses on the **linear
    model**. It says that the group variable has no effect, meaning all
    groups have the same average value. If the coefficients ($\beta_1$,
    $\beta_2$, etc.) are **zero**, then the model isn’t capturing any
    meaningful differences between groups.

-   **Using the F-statistic:** This version looks at **variance**. It
    compares the variation **between groups** to the variation **within
    groups**. If the F-value is **close to 1**, it means the differences
    between groups are about the same as random variation within groups,
    so there’s no real effect.

Both approaches test the same hypothesis—if we reject the null, we
conclude that at least one group is different from the others.
...
would still be one-way ANOVA if we had 4, 20 or 100 different types of
media.
...
**The diameters of bacterial colonies when grown in one of three
media**. A violin plot is an alternative to a boxplot for getting an
overview of our data before we begin analysis. Quick graphical overviews
help us identify any issues such as missing or extreme values. It also
tells us what to expect from the analysis. Adding both sugar and amino
acids to the medium seems to increase the colony diameter.
:::
...
**The a linear model fitted to the relationship between media and
diameter**. In an one-way ANOVA model with three groups, the model has
three coefficients (parameters) estimated from the data. The first
estimate is the intercept which is the mean of the first group. The
second estimate is the 'slope' to the second group which is what has to
added to the intercept to get the second group mean. The third estimate
is the 'slope' to the third group which is what has to added to the
intercept to get the third group mean. Note that y axis starts at 15 to
create more space for the annotations.
:::
...
If the ANOVA is *not* significant, no post-hoc is needed.

There are several possible post-hoc tests and we will be using Tukey's
HSD (honestly significant difference) test [@tukey1949] implemented in
...
'p.value' indicates whether that difference is significant. In this
case, two of the comparisons are significant:

-   control - sugar and amino acids added with *p* = `r p_cont_v_suaa`
-   sugar added - sugar and amino acids added with *p* =
    `r p_suga_v_suaa`
...
seems small but perhaps the residuals are more variable for the highest
mean.
:::
...
**Distribution of the residuals**. A histogram of the residuals allows
us to check the normality assumption. Here, the distribution fairly is
symmetrical and seems consistent with a normal distribution. This is a
good sign for the assumption of normally distributed residuals.
:::
...
*p* = `r p_value`). Post-hoc testing with Tukey's Honestly Significant
Difference test [@tukey1949] revealed the colony diameters were
significantly larger when grown with both sugar and amino acids
($\bar{x} \pm s.e$: `r suaa_m` $\pm$ `r suaa_se` mm) than with neither
(`r cont_m` $\pm$ `r cont_se` mm; *p* = `r p_cont_v_suaa`) or just sugar
(`r suga_m` $\pm$ `r suga_se` mm; *p* = `r p_suga_v_suaa`). See
@fig-culture.
...
`r df_residuals`; *p* = `r p_value`)). Post-hoc testing with Tukey's
Honestly Significant Difference test [@tukey1949] revealed the colony
diameters were significantly larger when grown with both sugar and amino
acids than with neither or just sugar. Data analysis was conducted in R
...
A significant Kruskal-Wallis tells us *at least two* of the groups
differ but where do the differences lie? The Dunn test [@dunn1964] is a
...
colonies (Kruskal-Wallis: *chi-squared* = `r chisq_value`; *df* =
`r df`; *p*-value = `r p_value`) with colonies growing significantly
better when both sugar and amino acids are added to the medium. Post-hoc
testing with the Dunn test [@dunn1964] revealed the colony diameters
were significantly larger when grown with both sugar and amino acids
(median = `r suaa_m` mm) than with neither (median = `r cont_m` mm; *p*
= `r p_cont_v_suaa`) or just sugar (median = `r suga_m` mm; *p* =
`r p_suga_v_suaa`). See @fig-culture-kw.
...
There was a significant effect of media on the diameter of bacterial
colonies (Kruskal-Wallis: *chi-squared* = `r chisq_value`; *df* =
`r df`; *p*-value = `r p_value`). Post-hoc testing with the Dunn test
