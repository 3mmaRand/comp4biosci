...
statistic, $F$ has a value of `r round(anova(mod)$F[1],2)`and degrees of
freedom of `r broom::tidy(anova(mod))$df[1]` and
`r broom::tidy(anova(mod))$df[3]`. This would be written as \$F = \$
`r round(anova(mod)$F[1],2)`; \$d.f.= \$
