...
chaffinch](https://en.wikipedia.org/wiki/Common_chaffinch) have been
described, based principally on the differences in the pattern and
colour of the adult male plumage [@suárez2009]. Two of groups (See
@fig-subspecies) of these subspecies are :
...
values in one group and any value in the other group.
...
2. If pairs of observations in the groups have something in common
   ...
   then those observations are not independent
   
3. A paired-samples test is used when the observations are not
   independent. 

4. A linear model with one explanatory variable with two groups and 
...
5. We can use `lm()` to do two-sample and paired sample tests. We can
   ...
   use `lm()`. The output of `lm()` is also more typical of the output of
...
7. We examine a histogram of the residuals and use the Shapiro-Wilk 
   normality tests to check the normality assumption. We  check the 
   variance of the residuals is the same for all fitted values with 
   a residuals vs fitted plot.

8. If the assumptions are not met, we can use alternatives known as 
