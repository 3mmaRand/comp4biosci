...
well in both subjects. In this scenario our null hypothesis, $H_0$, is 
that there is no difference between the Data Analysis and Biology marks
for a student.
...
statistic, $F$ has a value of `r round(anova(mod)$F[1],2)`and 
degrees of freedom of `r broom::tidy(anova(mod))$df[1]` and 
`r broom::tidy(anova(mod))$df[3]`. This would be written as 
$F = $ `r round(anova(mod)$F[1],2)`; $d.f.= $ 
`r broom::tidy(anova(mod))$df[1]`, `r broom::tidy(anova(mod))$df[3]`;
$p = $`r round(broom::tidy(anova(mod))$p.value[1], 3)` .
...
Biology (*t* = 2.47; *d.f.* = 9; *p* = 0.0355) with an average difference of 7%. Data analysis was conducted in R [@R-core] with tidyverse packages [@tidyverse].
...
non-parametric equivalent of the paired-samples *t*-test. This is 
often referred to as the paired-sample Wilcoxon test, or just the 
Wilcoxon test. 

The test is also applied in R with the `wilcox.test()` function but 
we add the `paired = TRUE` argument. We also have to give the two
datasets rather than using the "formula method" of `mark ~ subject`. 
This means it is useful to pivot the data to "wide" format.
