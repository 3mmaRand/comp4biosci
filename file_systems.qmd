...
One of the simplest types of file is a "text file", also known as ASCII files, which contains only text characters and no formatting, images or colours. Text files have many different extensions to indicate what the text content represents. A few of these are given in @tab-text-file-extensions.

```{r}
#| label: tab-text-file-extensions
#| echo: false
...
knitr::kable(ext, caption = "Some common text file extensions and the content the extension usually indicates.",
             col.names = c("Extension", "Content usually inside")) |> 
  kableExtra::kable_styling()

```

Plain text files are extremely portable which means they can be opened on any computer by a very large number of programs. Simple text editors which exist on any system like Windows Notepad or Mac's TextEdit will open text files where as they will not open program-specific files like, for example, .xlsx (Excel) or .docx (Word) files (Figure @fig-xlsx-in-notepad). It is usually easy for application designers to make an application manage plain text files. This means

![Notepad will open an Excel file but the contents are a mess of strange characters](images/xlsx-in-notepad.png){#fig-xlsx-in-notepad fig-alt="shows an xlsx file open in notpade with a whole load of strange characters. It is unreadable"}

Data is commonly held in text files because of their portability.

## Plain text files with markup

Text files can also include "markup" which are text characters used to annotate text to control how it is displayed or processed. Such files retain their portability and are human readable. You will have used them often!

```{r}
#| label: tab-mark-up-text-file-extensions
#| echo: false

ext <- data.frame(ext = c(".html", ".md", ".Rmd", ".JSON", ".tex", ".qmd"),
                  content = c("Hypertext Markup Language - for documents designed to be displayed in a web browser.", 
...
knitr::kable(ext, caption = "Some common markup file extensions and the content the extension usually indicates.",
...
It is easiest to picture a file system, or part of it, as a tree that starts at a directory and branches out from there. This is called a hierarchical structure. @fig-file-system shows an example of a hierarchical file structure that starts at a directory called `home`:
...
![File Explorer (Windows)](images/icon-file-explorer-windows.png){#fig-windows-explorer width="100"}

![Finder (Mac)](images/icon-finder-mac.png){#fig-mac-finder width="100"}
...
-   under View, in the Show/hide group, select the "File name extensions" check box @fig-show-extensions-windows

-   choose Finder \| Preferences \| Advanced then check "Show all filename extensions" box @fig-show-extensions-windows
