...
However, later we will also use the Spearman's rank correlation coefficient
which has fewer assumptions. This a more conservative approach.
...
a response variable. Pearson’s in the default test so we do not need to specify
it.

What do all these results mean?

The last line gives the correlation coefficient, $r$, that has been estimated 
from the data. Here, $r$ = `r cor.test(data = seeds, ~ compactness + kernel_width)$estimate` 
