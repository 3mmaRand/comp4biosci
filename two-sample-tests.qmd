...
of the adult male plumage [@suárez2009]. Two groups of these subspecies
are:
...
**Adult males of two subspecies of the common chaffinch, *Fringilla
coelebs*.** @fig-coelebs Coelebs group (Andreas Trepte, CC BY-SA 2.5
<https://creativecommons.org/licenses/by-sa/2.5>, via Wikimedia Commons)
and [-@fig-canariensis] the canariensis group (H. Zell, CC BY-SA 3.0
...
differ in mass. These two groups are independent - there is no link
between values in one group and any value in the other group. In this
scenario our null hypothesis, $H_0$, is that there is no difference
between the two subspecies in mass or that subspecies has no effect on
mass. This is written as: $H_0: \beta_1 = 0$.
...
**The mass of two subspecies of chaffinch**. A boxplot is a useful way
to get an overview of the data and helps us identify any issues such as
missing or extreme values. It also tells us what to expect from the
analysis. The canariensis group appears heavier than the coelebs group.
:::
...
**The a linear model fitted to the relationship between subspecies and
mass.** The model has two coefficients (parameters) estimated from the
data. Where the explanatory variable is categorical with two groups in a
linear model, the first estimate is the intercept which is the mean of
the first group. The second estimate is the slope which is what has to
added to the intercept to get the second group mean. Note that y axis
starts at 15 to create more space for the annotations.
:::
...
Here, the points are distributed similarly in each group. This is a good
sign for the assumption of homogeneity of variance.
:::
...
Canariensis chaffinches ($\bar{x} \pm s.e$: `r cana_m` $\pm$ `r cana_se`
g) were significantly heavier than Coelebs (`r coel_m` $\pm$ `r coel_se`
g; *t* = `r t_value`; *d.f.* = `r df_residuals`; *p* = `r p_value`). See
@fig-chaff.

```{r}
#| code-fold: true
#| warning: false
# extract p values from mod using broom for ease of reporting
mod_tidy <- broom::glance(mod)
p_value <- mod_tidy |> pull(p.value) |> round(4)
...
*d.f.* = `r df_residuals`; *p* = `r p_value`). Data analysis was
...
issues such as missing or extreme values. It also tells us what to
expect from the analysis. The mutant plants appear to have fewer leaves
than the wildtype plants.
:::
...
There are significantly more leaves on wildtype (median = `r wild_m`)
than mutant (median = `r mutant_m`) plants (Wilcoxon rank sum test: *W*
= `r W_value`; $n_1$ = `r wild_n`; $n_2$ = `r mutant_n`; *p* =
`r p_value`). See @fig-arabid.
...
*p* = `r p_value`). The heavy lines indicate the median number of
leaves, boxes indicate the interquartile range and whiskers the range.
...
`student`.[^two-sample-tests-1]

[^two-sample-tests-1]: You might like to try removing
...
function[^two-sample-tests-2] like this:

[^two-sample-tests-2]: This is not the only way to apply a paired test.
...
**Distribution of the residuals.** A histogram of the residuals allows
us to check the normality assumption. We can't draw strong conclusions
from this, but we do at least have a peak at 0.
:::
...
Biology (*t* = `r t_value`; *d.f.* = `r df_residuals`; *p* =
`r p_value`) with an average difference of `r diff`%. See @fig-marks
...
higher in Data Analysis than in Biology *t* = `r t_value`; *d.f.* =
`r df_residuals`; *p* = `r p_value`) with an average difference of
`r diff`%. Data analysis was conducted in R [@R-core] with tidyverse
packages [@tidyverse].
:::
