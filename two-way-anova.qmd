...
1.  **Use two-way ANOVA carefully**. Using the two-way ANOVA test and
    interpreting the results with caution is one valid approach. ANOVA
    is actually quite robust to violating the normality assumption (it
    performs quite well anyway) depending on how much the residuals
    deviate.
...
**The parasite load for two species of Littorina indicated by season.**
The fill colour indicates the season (Spring and Summer). Parasite load
seems to be higher for both species in the summer and that effect looks
bigger in *L.brevicula* - it has the lowest spring mean but the highest
summer mean. If the effect of season is different for the two species
there is an interaction between species and season.
:::
...
brev_spring_m <- peri_summary |> 
  filter(species == "Littorina brevicula", season == "Spring") |> 
  pull(mean) |> 
  round(1)

brev_spring_se <- peri_summary |> 
  filter(species == "Littorina brevicula", season == "Spring") |> 
  pull(se) |> 
  round(2)

brev_summer_m <- peri_summary |> 
  filter(species == "Littorina brevicula", season == "Summer") |> 
  pull(mean) |> 
  round(1)

brev_summer_se <- peri_summary |> 
  filter(species == "Littorina brevicula", season == "Summer") |> 
  pull(se) |> 
  round(2)

litt_spring_m <- peri_summary |> 
  filter(species == "Littorina littorea", season == "Spring") |> 
  pull(mean) |> 
  round(1)

litt_spring_se <- peri_summary |> 
  filter(species == "Littorina littorea", season == "Spring") |> 
  pull(se) |> 
  round(2)

litt_summer_m <- peri_summary |> 
  filter(species == "Littorina littorea", season == "Summer") |> 
  pull(mean) |> 
  round(1)

litt_summer_se <- peri_summary |> 
  filter(species == "Littorina littorea", season == "Summer") |> 
  pull(se) |> 
  round(2)
...
minus *L.littorea* is `r brev_spring_m - litt_spring_m` in the spring
but `r brev_summer_m - litt_summer_m` in summer.
...
```{r}
#| echo: false
# extract values from anova for ease of reporting
anova_results <- broom::tidy(anova(mod))

season_f <- anova_results |> 
  filter(term == "season") |> 
  pull(statistic) |> 
  round(1)

season_df1 <- anova_results |> 
  filter(term == "season") |> 
  pull(df) 

season_p <- anova_results |> 
  filter(term == "season") |> 
  pull(p.value) |> 
  (\(x) ifelse(x < 0.0001, "< 0.0001", round(x, 4)))()

interaction_f <- anova_results |> 
  filter(term == "species:season") |> 
  pull(statistic) |> 
  round(1)

interaction_df1 <- anova_results |> 
  filter(term == "species:season") |> 
  pull(df) 
...
# get residual df
residual_df <- anova_results |> 
  filter(term == "Residuals") |> 
  pull(df) 

```

The parasite load is significantly greater in Summer (*F* =
`r season_f`; *d.f.* = `r season_df1`, `r residual_df`; *p*
`r season_p`) but this effect differs between species (*F* =
`r interaction_f`; *d.f.* = `r interaction_df1`, `r residual_df`; *p* =
`r interaction_p`) with a greater increase in parasite load in
*L.brevicula* than in *L.littorea*.
...
#| echo: false
#| warning: false
# extract post-hoc p-values for ease of reporting
posthoc_results <- emmeans(mod, ~ species * season) |> 
  pairs() |> 
  as.data.frame()

p_brev_spring_summer <- posthoc_results |> 
  filter(contrast == "Littorina brevicula Spring - Littorina brevicula Summer") |> 
  pull(p.value) |> 
  (\(x) ifelse(x < 0.0001, "< 0.0001", round(x, 4)))()

p_brev_spring_litt_summer <- posthoc_results |> 
  filter(contrast == "Littorina brevicula Spring - Littorina littorea Summer") |> 
  pull(p.value) |> 
  (\(x) ifelse(x < 0.0001, "< 0.0001", round(x, 4)))()

p_litt_spring_brev_summer <- posthoc_results |> 
  filter(contrast == "Littorina littorea Spring - Littorina brevicula Summer") |> 
  pull(p.value) |> 
  (\(x) ifelse(x < 0.0001, "< 0.0001", round(x, 4)))()
...
-   *L.brevicula* in the Spring and Summer `r p_brev_spring_summer`
-   *L.brevicula* in the Spring and *L.littorea* in the Summer
    `r p_brev_spring_litt_summer`
-   *L.littorea* in the Spring *L.brevicula* in the Summer
    `r p_litt_spring_brev_summer`
...
**Residuals against the fitted values.** This plot shows whether the
points are distributed similarly in each group. Any difference seems
small but perhaps the residuals are less variable for the lowest mean.
:::
...
**A histogram of residuals** is symetrical and seems consistent with a
normal distribution. This is a good sign for the assumption of normally
...
The parasite load was significantly greater in Summer (*F* =
`r season_f`; *d.f.* = `r season_df1`, `r residual_df`; *p*
`r season_p`) but this effect differed between species (*F* =
`r interaction_f`; *d.f.* = `r interaction_df1`, `r residual_df`; *p* =
`r interaction_p`) with a greater increase in parasite load in
*L.brevicula* (from $\bar{x} \pm s.e$: `r brev_spring_m` $\pm$
`r brev_spring_se` units to `r brev_summer_m` $\pm$ `r brev_summer_se`
units) than in *L.littorea* (from `r litt_spring_m` $\pm$
`r litt_spring_se` units to `r litt_summer_m` $\pm$ `r litt_summer_se`
units). See @fig-para.
...
ggplot() +
  ...
  annotate("text",
           x = 1.25,  y = 119,
           label = glue::glue("italic(p) {p_brev_spring_summer}"),
           parse = TRUE) +
  ...
  annotate("text", x = 1.5,  y = 129,
           label = glue::glue("italic(p) == {p_brev_spring_litt_summer}"),
           parse = TRUE) +
  ...
  annotate("text", x = 1.5,  y = 109,
           label = glue::glue("italic(p) == {p_litt_spring_brev_summer}"),
           parse = TRUE) +
  theme_classic() +
...
Summer (*F* = `r season_f`; *d.f.* = `r season_df1`, `r residual_df`;
*p* `r p_brev_spring_summer`) but this effect differed between species
(*F* = `r interaction_f`; *d.f.* = `r interaction_df1`, `r residual_df`;
*p* = `r interaction_p`) with a greater increase in parasite load in
*L.brevicula* than in *L.littorea*. Data analysis was conducted in R
[@R-core] with tidyverse packages [@tidyverse]. Post-hoc analysis was
carried out with Tukey's Honestly Significant Difference test
[@tukey1949].
:::
